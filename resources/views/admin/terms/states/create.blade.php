            ...
            <form class="form-valide" id="state-form" action="@yield('state_action')" method="post">
                ...
                <div class="row">
                    <div class="col-xl-12">
                        ...
                        <div class="form-group row">
                            <label class="col-lg-2 col-form-label" for="state-parent">state Parent
                            </label>
                            <div class="col-lg-10">
                                <select class="form-control multi-select" id="state-parent" name="parent_state" data-existed_parent_facitity="{{$state->parent_state ?? ''}}">
                                    @isset($states)
                                    <option value="">Select State Parent</option>
                                    @foreach($states as $s_p)
                                    <option value="{{$s_p->id}}" {!!get_edit_select_post_types_old_value($s_p->id, $state->parent_state ?? "",'select')!!} >{{$s_p->name}}</option>
                                    @endforeach
                                    @endisset
                                </select>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-10">
                                ...
                                <label class="col-form-label">
                                    <input type="radio" name="status" value="1" {!!get_edit_select_check_pvr_old_value('status',$state ?? "",'status',1, 'chacked')!!}>&nbsp;Active</label>
                                    <label class="col-form-label">
                                        <input type="radio" name="status" {!!get_edit_select_check_pvr_old_value('status',$state ?? "",'status',0, 'chacked')!!} value="0">&nbsp;Inactive</label>
                                    </div>
                                </div>
