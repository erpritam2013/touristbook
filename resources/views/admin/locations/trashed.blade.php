...
<div class="container-fluid">
   ...
   <div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                ...
                <div align="right" class="all-a">
                    ...
                    <form id='bulk_force_delete_entity_form' method="POST" action="{{route('admin.location.bulk-force-delete')}}" style="display: none" data-text="page">
                      {{ csrf_field() }}
                    ...
                    <form id='bulk_restore_entity_form' method="POST" action="{{route('admin.location.restore-bulk')}}" style="display: none" data-text="page">
                      {{ csrf_field() }}
                  ...
                  <a href="javascript:void(0);" class="btn btn-outline-danger btn-xs"  onclick="confirm('Are You Want To Empty Trash')?jQuery('#empty_trashed_entity_form').submit():false">Empty Trash</a>
                    <form id='empty_trashed_entity_form' method="POST" action="{{route('admin.location.empty-trashed')}}" style="display: none" data-text="post">
                      {{ csrf_field() }}
                      {{method_field('DELETE')}}
                  </form>
                  <form id='all_restore_entity_form' method="POST" action="{{route('admin.location.restore-all')}}" style="display: none" data-text="page">
                      {{ csrf_field() }}
       ...
       <form id='restore_entity_form' method="POST" action="{{route('admin.location.trashed')}}" style="display: none">
          {{ csrf_field() }}
