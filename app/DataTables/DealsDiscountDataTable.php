...
class DealsDiscountDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
         return (new EloquentDataTable($query))->addIndexColumn()->addColumn('icon',function($row){
                return get_fontawesome_icon_html($row->icon,'fa-lg');
                    ...
                    $html = ' <a href="'.route("admin.terms.deal-discounts.edit",$row->id).'" class="btn btn-primary" title="Edit"><i class="fa fa-edit"></i></a>';
                    $html .= '<a href="'.route("admin.terms.deal-discounts.show",$row->id).'" class="btn btn-info" title="View"><i class="fa fa-file"></i></a>';
                    $html .= '<a href="javascript:void(0);" class="btn btn-danger del_entity_form" title="Delete" item_id="'.$row->id.'" data-text="deal & discount"><i class="fa fa-trash"></i></a>';
                    return $html;
                    ...
                    return '<input data-id="'.$row->id.'" class="toggle-class" type="checkbox" data-size="sm" data-onstyle="success" data-offstyle="danger" data-toggle="toggle" data-url="'.route("admin.terms.changeStatusDealsDiscount").'" data-on="Active" data-off="InActive" '.$checked.' '.json_encode($row).'>';
     ...
     *
     * @param \App\Models\Terms\DealsDiscount $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(DealsDiscount $model): QueryBuilder
    {
    ...
    public function getColumns(): array
    {
        return [
            ...
            ->exportable(false)
            ->printable(false),
            Column::make('deals_discount_type')->title('Type'),
            Column::make('status'),
    ...
    {
        return 'DealsDiscount_' . date('YmdHis');
    }
}
