...
class ActivityZoneDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))->addIndexColumn()->addColumn('action', function ($row) {
                    $html = ' <a href="'.route("admin.activity-zones.edit",$row->id).'" class="btn btn-primary" title="Edit"><i class="fa fa-edit"></i></a>';
                    //$html .= '<a href="'.route("admin.activity-zones.show",$row->id).'" class="btn btn-info" title="View"><i class="fa fa-file"></i></a>';
                   ...
                   $editor_name = (!empty($row->editor_name()) && $row->isEditing()) ? '<p class="edit-name">( '.$row->editor_name().' )</p>' : '';
                    ...
                    return (!empty($row->user))?'<a href="'.route('admin.activity-zones.index').'?user='.$row->user->id.'" target="_blank" style="color:#07509e">'.'#'.$row->user->id.' '.$row->user->name.'</a> : ':null;
                    }
                    ...
                    return '<input data-id="'.$row->id.'" class="toggle-class" type="checkbox" data-size="sm" data-onstyle="success" data-offstyle="danger" data-toggle="toggle" data-url="'.route("admin.changeStatusActivityZone").'" data-on="Active" data-off="InActive" '.$checked.'>';
     ...
     *
     * @param \App\Models\ActivityZone $model
    ...
    {
        return 'ActivityZone_' . date('YmdHis');
    }
}
