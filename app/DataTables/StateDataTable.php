...
class StateDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
         return (new EloquentDataTable($query))->addIndexColumn()->addColumn('icon',function($row){
                return get_fontawesome_icon_html($row->icon,'fa-lg');
                    ...
                    return '<input data-id="'.$row->id.'" class="toggle-class" type="checkbox" data-size="sm" data-onstyle="success" data-offstyle="danger" data-toggle="toggle" data-url="'.route("admin.terms.changeStatusState").'" data-on="Active" data-off="InActive" '.$checked.' '.json_encode($row).'>';
     ...
     *
     * @param \App\Models\Terms\State $model
